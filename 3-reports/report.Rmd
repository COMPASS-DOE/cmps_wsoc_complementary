...
nmr_relabundance_combined = 
  ...
  adonis2(relabund_wide %>% dplyr::select(where(is.numeric))  ~ 
            (extraction + site + transect_location + horizon),
          by = "margin",
          data = relabund_wide) %>% 
    knitr::kable()
...
For WATER extracts, transect location accounted for only 10% of total variability.

For SPE extracts, transect location accounted for 41% of total variability.
